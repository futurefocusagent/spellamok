...
export default function App() {
  ...
  function applyNewState(s: GameState, prevState: GameState | null) {
    // If computer just played a move (with placements), animate them in
  ...
  function placeOnCell(rackIndex: number, tile: Tile, r: number, c: number) {
    ...
    if (m.has(`${r}-${c}`)) return;
    // Block zone check (only zones from opponent affect us)
    ...
    // Remove if already pending from same rackIndex
  ...
  // Build a "view board" that hides cells in hiddenCells (during computer animation)
  ...
  // Filter computer's visible last move placements (for animations)
  ...
  // Computer rack to display: if peek active, show real letters; else dimmed placeholders
      ...
      <div className="min-h-screen flex flex-col gap-3 px-3 py-3 sm:py-4 max-w-3xl mx-auto">
        <header className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs px-2 py-1 rounded border border-neutral-700 bg-neutral-900 hover:bg-neutral-800"
          >
            New Game
          </button>
        </header>
        ...
        {/* Computer rack (peek dim/visible) */}
